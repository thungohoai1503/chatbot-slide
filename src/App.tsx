...
- Phương thức Thanh toán (PTTT v7d):
  ...
  useEffect(() => {
    ...
    // Load auto-scanned topics from public/topics.json (generated by scripts/scan-folder.cjs)
    const loadScannedTopics = async () => {
      try {
        ...
        for (const topic of data.topics) {
          newKeywords[topic.name] = topic.keywords || [];
          newImages[topic.name] = (topic.images || []).map(
            (img: string) => `/images/${img}`
          );
        }

        setTopicKeywords((prev) => ({ ...prev, ...newKeywords }));
        setCustomTopicImages((prev) => ({ ...prev, ...newImages }));
      } catch (err) {
        console.warn("Không tải được topics.json:", err);
      }
    };
    loadScannedTopics();
  }, []);
